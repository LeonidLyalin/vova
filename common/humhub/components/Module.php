...
use Yii;
use common\humhub\models\ModuleEnabled;
...
class Module extends \yii\base\Module
{
    ...
    public function disable()
    {
        ...
        foreach (\common\humhub\modules\content\models\ContentContainerSetting::findAll(['module_id' => $this->id]) as $containerSetting) {
            $containerSetting->delete();
        }

        foreach (\common\humhub\models\Setting::findAll(['module_id' => $this->id]) as $containerSetting) {
            $containerSetting->delete();
        }

        foreach (\common\humhub\modules\user\models\Module::findAll(['module_id' => $this->id]) as $userModule) {
            $userModule->delete();
        }

        foreach (\common\humhub\modules\space\models\Module::findAll(['module_id' => $this->id]) as $spaceModule) {
            $spaceModule->delete();
        }
    ...
    public function migrate()
    {
        ...
        if (is_dir($migrationPath)) {
            \common\humhub\commands\MigrateController::webMigrateUp($migrationPath);
        }
    }
